...
class DataArguments:
    ...
    per_lang_max_examples: Optional[int] = field(
        default=1000,
        metadata={"help": "Maximum number of examples to use per loaded XNLI dataset"},
    )
...
def load_checkpoint(checkpoint_dir: str):
    ...
    model = AutoModelForSequenceClassification.from_pretrained(checkpoint_dir).to(
...
def main():
    ...
    label_list_train = train_datasets[0].features["label"].names
    num_labels = len(label_list_train)
    ...
    example_datasets = [[] for _ in range(len(train_datasets[0]))]
    for idx, example in enumerate(zip(*train_datasets)):
